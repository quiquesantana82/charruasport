  ...
  for (const p of players) {
    try {
      ...
      if (p.stats_source === 'fotmob') {
        ...
        const fm = await fetchFotmob(p.fotmob_id);
        if (!fm || fm.motivo) {
          results.push({ jugador: p.nombre, ok: false, motivo: (fm && fm.motivo) || 'Sin datos en FotMob' });
          continue;
        }
        const temporadasFM = fm.temporadas;
...
async function fetchFotmob(fotmobId) {
  let r;
  try {
    r = await fetch(`https://www.fotmob.com/api/playerData?id=${fotmobId}`, {
      headers: {
    ...
    return { motivo: 'FotMob inaccesible: ' + e.message };
  }
  if (!r.ok) return { motivo: 'FotMob respondió HTTP ' + r.status };
  const texto = await r.text();
  let j;
  try { j = JSON.parse(texto); } catch {
    return { motivo: 'FotMob devolvió contenido no JSON: "' + texto.slice(0, 80).replace(/\s+/g, ' ') + '..."' };
  }

  const ml = j.mainLeague;
  if (!ml || !ml.stats || !ml.stats.length) {
    return { motivo: 'Respuesta sin mainLeague.stats. Campos recibidos: ' + Object.keys(j).slice(0, 10).join(', ') };
  }
  ...
  if (!partidos) return { motivo: 'mainLeague sin partidos. Stats recibidas: ' + Object.keys(map).join(', ') };
